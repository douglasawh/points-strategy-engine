...
from ..utils.date_utils import parse_date_or_none, validate_date_range
from ..engine.models import Trip, Recommendation
from ..engine.scorer import score_flight, score_stay
from ..engine.render_markdown import render_markdown
from ..providers.flights.delta_msp_hnd import propose_flights
from ..providers.hotels.hyatt import load_calendars_for_trip, allocate_hyatt_stay

def prompt_if_missing(args) -> tuple[date|None, date|None, bool, str, list[str]]:
    # Dates
    ...
    while (start is None or end is None) and not args.noninteractive:
        print("Enter trip dates (examples: 2027-11-20 or Nov 20 2027). Leave blank to skip.")
        if start is None:
            start = parse_date_or_none(input("Start date: ").strip())
        if end is None:
            end = parse_date_or_none(input("End date: ").strip())
        if start and end:
            ok, msg = validate_date_range(start, end)
            if not ok:
                print("Error:", msg); start, end = None, None
...
def main():
    ...
    ap.add_argument("--end", help="End date (e.g., 2027-12-02)")
    ...
    ap.add_argument("--calendar-mode", choices=["import","fixture"], default="fixture",
                    help="How to load Hyatt points calendars.")
    ap.add_argument("--calendar-file", action="append",
                    help="Repeatable: hotel_name=path (for --calendar-mode=import)")
    ...
    trip = Trip(
        ...
        notes="Planned via points-strategy-engine"
    )
